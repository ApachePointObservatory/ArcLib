...
// +------------------------------------------------------------------------------------+
#define VERIFY_CLASS_PTR()													\
{																			\
	if ( g_pDevice == nullptr )												\
	{																		\
		throwArcGen3Error( "Invalid class object pointer!" );				\
	}																		\
}
...
// +------------------------------------------------------------------------------------+
const unsigned int DEVICE_NOPARAM = arc::gen3::CArcDevice::NOPARAM;
...
GEN3_CARCDEVICE_API const char* ArcDevice_ToString( int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		ArcSprintf( g_szTmpBuf, ARC_MSG_SIZE, "%s", g_pDevice->toString().data() );
	}
...
GEN3_CARCDEVICE_API const char** ArcDevice_GetDeviceStringList( int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		if ( ArcDevice_DeviceCount() > 0 )
		{
			auto pPCIList  = arc::gen3::CArcPCI::getDeviceStringList();
			auto pPCIeList = arc::gen3::CArcPCIe::getDeviceStringList();
			...
			if ( g_pDeviceStringList == nullptr )
			{
				throwArcGen3Error( "Failed to allocate list memory!" );
			}
			...
			for ( std::uint32_t i = 0; i < arc::gen3::CArcPCI::deviceCount(); i++ )
			{
				...
				ArcSprintf( &g_pDeviceStringList[ i ][ 0 ], 100, "%s", pPCIList.lock()[ i ].c_str() );
			}
			...
			for ( std::uint32_t i = arc::gen3::CArcPCI::deviceCount(); i < ArcDevice_DeviceCount(); i++ )
			{
				...
				ArcSprintf( &g_pDeviceStringList[ i ][ 0 ], 100, "%s", pPCIeList.lock()[ i - arc::gen3::CArcPCI::deviceCount() ].c_str() );
			}
		}
	}
	catch ( const std::exception& e )
...
GEN3_CARCDEVICE_API void ArcDevice_Open( unsigned int uiDeviceNumber, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		auto tFromCharsResult = std::from_chars( pTokens->at( 2 ).data(), ( pTokens->at( 2 ).data() + pTokens->at( 2 ).size() ), uiDevNum );

		if ( tFromCharsResult.ec == std::errc::invalid_argument )
		{
			throwArcGen3Error( "Failed to convert device number" );
		}

		if ( tFromCharsResult.ec == std::errc::result_out_of_range )
		{
			throwArcGen3LengthError( "Failed to convert device number" );
		}
		...
		{
			throwArcGen3Error( "( ArcDevice_Open ): No ARC device found!" );
		}

		VERIFY_CLASS_PTR()

		g_pDevice->open( uiDevNum );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
GEN3_CARCDEVICE_API void ArcDevice_Open_I( unsigned int uiDeviceNumber, unsigned int uiBytes, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		{
			throwArcGen3Error( "( ArcDevice_Open_I ): No ARC device found!" );
		}

		VERIFY_CLASS_PTR()

		g_pDevice->open( uiDevNum, uiBytes );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
void ArcDevice_Open_II( unsigned int uiDeviceNumber, unsigned int uiRows, unsigned int uiCols, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pDevice->open( uiDevNum, uiRows, uiCols );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
...
GEN3_CARCDEVICE_API void ArcDevice_LoadControllerFile( const char* pszFilename, unsigned int uiValidate, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pDevice->loadControllerFile( pszFilename, ( uiValidate == 1 ? true : false ) );
	}
	catch ( const std::exception& e )
	{
		*pStatus = ARC_STATUS_ERROR;
