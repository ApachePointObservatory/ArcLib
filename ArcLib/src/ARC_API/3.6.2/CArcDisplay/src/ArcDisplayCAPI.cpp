...
// |                                                                                    |
// |  <IN> -> uiMSDelay - The amount of time to delay before attempting to              |
// |                      access the application. Default: 1 second                     |
// |  <OUT> -> pStatus - Status equals ARC_STATUS_OK or ARC_STATUS_ERROR                |
// +------------------------------------------------------------------------------------+
GEN3_CARCDISPLAY_API void ArcDisplay_Launch( unsigned int uiMSDelay, int* pStatus )
{
	*pStatus = ARC_STATUS_OK;
	...
	try
	{
		...
		g_pCDisplay.get()->launch( uiMSDelay );
	}
