	...
	namespace gen3
	{
		...
		public:
			...
			*/
			 ...
			 */
			void launch( const std::uint32_t uiMSDelay = 1000 );
			 ...
			 *  @throws std::runtime_error
			 */
			void clear( const std::int32_t iFrame = -1 );
		...
		private:
			 ...
			 *  @return <i>true</i> if the DS9 process is running and valid; <i>false</i> otherwise.
			 */
			constexpr bool verifyPID( void ) noexcept;
			...
			bool getModuleDirectory( TCHAR* pszBuf, const size_t tBufSize );

#else

			bool getModuleDirectory( char* pszBuf, const size_t tBufSize );
