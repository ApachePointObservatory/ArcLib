...
namespace arc
{
	namespace gen3
	{
		...
		void CArcPluginManager::getDirList( const std::string& sPath, std::vector<std::string>& vDirs )
		{
				...
				{
					throwArcGen3Error( "Invalid File Handle. GetLastError reports: %e", GetLastError() );
				}
				...
				if ( pDir == nullptr )
				{
					throwArcGen3Error( "Failed to open dir: %s", sPath.c_str() );
				}
		...
		void CArcPluginManager::loadCustomLibrary( const std::string sLibPath, const std::string sLibName )
		{
				...
				{
					throwArcGen3Error( "Failed to set library path [ %s ]!", sLibPath.c_str() );
				}
			#endif
			...
			{
				throwArcGen3Error( "Failed to find load library [ PATH: %s, LIB: %s ]! GetLastError: %e",
						sLibPath.c_str(),
						sLibName.c_str(),
						ArcSysErrorCode() );
			}
