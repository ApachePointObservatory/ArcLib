...
#define VERIFY_INSTANCE_HANDLE( handle )	if ( handle == 0 ||															\
											...
											{																			\
												throwArcGen3Error( "Invalid deinterlace handle: 0x%X", ulHandle );					\
											}
...
GEN3_CARCDEINTERLACE_API unsigned long long	ArcDLace_getInstance( unsigned int uiBpp, ArcStatus_t* pStatus )
{
		...
		try
	{
		...
		{
			throwArcGen3InvalidArgument( "Invalid bits-per-pixel setting [ %d ]. Must be DLACE_BPP16 or DLACE_BPP32.", uiBpp );
		}
...
GEN3_CARCDEINTERLACE_API const char** ArcDLace_pluginList( unsigned long long ulHandle, unsigned int uiPlugin, ArcStatus_t* pStatus )
{
		...
		try
	{
		...
		{
			throwArcGen3InvalidArgument( "Invalid plugin value [ %u ], expected range: 0 to %u", uiPlugin, pluginManager->pluginCount() );
		}
	...
	unsigned int uiRows, const char* pszAlg, unsigned int uiArg, ArcStatus_t* pStatus )
{
		...
		try
	{
		...
		if ( uiPlugin >= pluginManager->pluginCount() )
		{
			throwArcGen3InvalidArgument( "Invalid plugin value [ %u ], expected range: 0 to %u", uiPlugin, g_pDLace16->getPluginManager()->pluginCount() );
		}
...
//		if ( uiPlugin >= pluginManager->pluginCount() )
//		{
//			throwArcGen3Error( "Invalid plugin value [ %u ], expected range: 0 to %u", uiPlugin, g_pDLace16->getPluginManager()->pluginCount() );
//		}
//
...
////			{
////				throwArcGen3Error( "Invalid class pointer!" );
////			}
////
////			if ( uiArg == DLACE_NO_ARG )
////			{
