...
#define VERIFY_INSTANCE_HANDLE( handle )	if ( handle == 0 ||															\
											...
											{																			\
												throwArcGen3Error( "Invalid FITS file handle: 0x%X", ulHandle );					\
											}
...
GEN3_CARCFITSFILE_API unsigned long long ArcFitsFile_getInstance( unsigned int uiBpp, ArcStatus_t* pStatus )
{
	...
	try
	{
		...
		{
			throwArcGen3Error( "Invalid bits-per-pixel setting [ %d ]. Must be FITS_BPP16 or FITS_BPP32.", uiBpp );
		}
...
// |  <OUT> -> pStatus  - Success state; equals ARC_STATUS_OK or ARC_STATUS_ERROR.                            |
// +----------------------------------------------------------------------------------------------------------+
GEN3_CARCFITSFILE_API void ArcFitsFile_getParameters( unsigned long long ulHandle, unsigned long* pNaxes, unsigned int* pNaxis, unsigned int* pBpp, ArcStatus_t* pStatus )
{
	INIT_STATUS( pStatus, ARC_STATUS_OK )
...
GEN3_CARCFITSFILE_API void ArcFitsFile_writeSubImage( unsigned long long ulHandle, void* pBuf, long llX, long llY, long urX, long urY, ArcStatus_t* pStatus )
{
		...
		try
	{
		VERIFY_INSTANCE_HANDLE( ulHandle )
			...
			if ( ulHandle == reinterpret_cast< std::uint64_t >( g_pFits16.get() ) )
			{
				g_pFits16->writeSubImage( reinterpret_cast< arc::gen3::fits::BPP_16* >( pBuf ),
										  arc::gen3::fits::MAKE_POINT( llX, llY ),
										  arc::gen3::fits::MAKE_POINT( urX, urY ) );
			}

			else
			{
				g_pFits32->writeSubImage( reinterpret_cast< arc::gen3::fits::BPP_32* >( pBuf ),
										  arc::gen3::fits::MAKE_POINT( llX, llY ),
										  arc::gen3::fits::MAKE_POINT( urX, urY ) );
			}
	}
...
GEN3_CARCFITSFILE_API void* ArcFitsFile_readSubImage( unsigned long long ulHandle, long llX, long llY, long urX, long urY, ArcStatus_t* pStatus )
{
	...
	try
	{
		...
		if ( ulHandle == reinterpret_cast< std::uint64_t >( g_pFits16.get() ) )
		{
			auto pSub16 = g_pFits16->readSubImage( arc::gen3::fits::MAKE_POINT( llX, llY ), arc::gen3::fits::MAKE_POINT( urX, urY ) );

			if ( pSub16 != nullptr )
			{
				auto iSize = ( static_cast<std::size_t>( std::abs( urX - llX ) ) * static_cast< std::size_t >( std::abs( urY - llY ) ) * sizeof( arc::gen3::fits::BPP_16 ) );
		...
		{
			auto pSub32 = g_pFits32->readSubImage( arc::gen3::fits::MAKE_POINT( llX, llY ), arc::gen3::fits::MAKE_POINT( urX, urY ) );

			if ( pSub32 != nullptr )
			{
				auto iSize = ( static_cast< std::size_t >( std::abs( urX - llX ) ) * static_cast< std::size_t >( std::abs( urY - llY ) ) * sizeof( arc::gen3::fits::BPP_32 ) );
	...
	try
	{
		VERIFY_INSTANCE_HANDLE( ulHandle )
			...
			if ( ulHandle == reinterpret_cast< std::uint64_t >( g_pFits16.get() ) )
			{
				...
				if ( pSub16 != nullptr )
				{
					auto iSize = ( static_cast< std::size_t >( g_pFits16->getCols() ) *
								   static_cast< std::size_t >( g_pFits16->getRows() ) *
								   sizeof( arc::gen3::fits::BPP_16 ) );
			...
			else
			{
				...
				if ( pSub32 != nullptr )
				{
					auto iSize = ( static_cast< std::size_t >( g_pFits32->getCols() ) *
								   static_cast< std::size_t >( g_pFits32->getRows() ) *
								   sizeof( arc::gen3::fits::BPP_32 ) );
